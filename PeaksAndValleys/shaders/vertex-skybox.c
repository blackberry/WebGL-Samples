...
varying vec2 vTextureCoord;		// A variable to hold the texture coordinates; to be passsed to the fragment shader.

void main(void) {
	/* Combine our matrices to generate a position in screen-space. */
	gl_Position = uPMatrix * uMVMatrix * vec4(aVertexPosition, 1.0);

	/* Store our texture variable for use by our fragment shader. */
